...
class Librus {
  ...
  constructor(cookies, options = {}) {
    ...
    // importSession rebuilds the caller itself when it adopts a jar; only
    // build one here if it didn't (no session given, or an unusable one).
    const restored = this.options.session
      ? this.importSession(this.options.session)
      : false;
    if (!restored) this._callerReady = this._initializeCaller();
  ...
  async authorize(login, pass) {
    ...
    // NOTE: authorize() must never route through _request(). Both URLs it
    // visits (/loguj/portalRodzina and /OAuth/Authorization) match
    // looksLikeLoginPage's own signals, so doing so would make every login
    // throw LibrusSessionExpiredError and loop.
  ...
  /**
   * Is there still a usable Synergia session in this jar?
   *
   * The session cookie is short-lived (~10 minutes). A jar whose other
   * cookies survive will still get an authenticated-looking 200 back from
   * Librus - with an access-denied body and no scrapeable content - so
   * checking for the session cookie is the only reliable signal that a
   * restored or cached client can actually fetch anything.
   *
   * @returns {Promise<Boolean>}
   */
  async hasLiveSession() {
    const cookies = await this.cookie.getCookies(config.page_url);
    return cookies.some((cookie) => cookie.key === config.session_cookie);
  }
   ...
   * @private
   */
  async _request(method, apiFunction, data, blank) {
    // On the production path _initializeCaller awaits a dynamic import, so
    // this.caller is assigned asynchronously and a client that was just
    // constructed does not have one yet. Wait for it here rather than in
    // every resource module.
    if (!this.caller) await (this._callerReady || this._initializeCaller());
   ...
   */
  async _getFile(path) {
    // Same readiness guard as _request - this.caller may not exist yet on a
    // freshly constructed client (see _initializeCaller).
    if (!this.caller) await (this._callerReady || this._initializeCaller());
   ...
   * Only consulted once a login has already failed (no `goTo` redirect
   * target came back), so a false positive can only change which error
   * message is thrown - never block a login that actually succeeded.
